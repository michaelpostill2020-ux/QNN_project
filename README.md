...
├── config.yaml                 # Master configuration file (features, targets, activations, paths)
├── setup_env.sh                # Multi-platform environment installation script
├── generate_mock_data.py       # Physics-based synthetic ROOT file generator
├── run_regression.py           # Core execution runner (ETL conversion, training, evaluation)
├── qnn_regression/             # Core library package
│   ├── __init__.py
│   ├── config.py               # YAML config parser & fallback handler
│   ├── data.py                 # Dynamic ETL pipeline (ROOT to H5 with transformations)
│   ├── models.py               # Classical Neural Nets and Quantum PQCs
│   └── utils.py                # Visualization and plotting suite
├── tests/                      # Sandbox testing suite
│   ├── test_config.yaml        # Lightweight config profile for CI
│   ├── test_config.py          # Unit tests for YAML loading
│   └── test_integration.py     # End-to-end integration test (Mock ROOT -> ETL -> Models)
...
# evlaluate results of with Pearson correlations and feature importance. Also compares model with and without quantum boost.
